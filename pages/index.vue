...
<template>
  <v-container class="py-8">
    ...
    <!-- Brak streamów -->
    <v-sheet v-if="!streamStore.streams || streamStore.streams.length === 0"
    ...
    <v-row>
      <v-col v-for="stream in streamStore.streams" :key="stream.id" cols="12" sm="6" md="4" lg="3">
        <v-card @click="goToStream(stream.username)" class="stream-card" :class="{ 'live-border': stream.isLive }"
          hover>
          <!-- Thumbnail z indykatorem live -->
          <div class="thumbnail-container">
            <v-img :src="stream.thumbnailUrl || '/Buddyshare.svg'" height="160px" cover
