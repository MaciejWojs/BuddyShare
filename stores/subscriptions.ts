export const useSubscriptionsStore = defineStore("Subscriptions", () => {
  ...
  async function removeSubscription(username: string) {
    ...
    const idx = subscriptions.value.findIndex(
      (sub: any) => sub.streamerUsername === username
    );
    if (idx !== -1) {
  ...
  async function addSubscription(username: string) {
    if (!import.meta.client) return;
    await authStore.waitUntilReady?.();
    if (!authStore.authenticated) {
      console.log("User not authenticated, skipping addSubscription.");
      return;
    }
    console.log("Adding subscription for user:", username);
    const { data, error } = await streamers.subscribe(username);
    if (error.value) throw error.value;
    if (!data.value) {
      console.error("No data received for addSubscription.");
      return;
    }
    subscriptions.value = null;
    subscriptions.value = data.value;
    console.log("Added subscription for user:", username);
  }
